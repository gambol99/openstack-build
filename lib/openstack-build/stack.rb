...
module OpenstackBuild
  class Stack
    ...
    def launch(hostname, o = {}, &block)
      ...
      raise ArgumentError, 'the networks field is empty' if o.networks.empty?
      raise ArgumentError, 'security group should be a array' unless o.security_group.is_a? Array
      raise ArgumentError, 'the security_group field is empty' if o.security_group.empty?
    ...
    def addresses(name, interval = 0.2, timeout = 60)
      ...
      begin
        ...
        raise Exception, "we have timed out attempting to acquire the instance addresses: #{e.message}"
      end
      list
    end
    ...
    def associate(hostname, floating_ip = nil, ip_address = nil)
      ...
      raise ArgumentError, "the hostname: #{hostname} does not have any network ports" if instance_ports.empty?
    ...
    def connection(openstack)
      ...
      @stack[:compute] = ::Fog::Compute.new(:provider => :OpenStack,
                                            ...
                                            :openstack_auth_key => openstack.api_key,
